...
@section('content')
    <section class="relative isolate min-h-[760px] overflow-hidden bg-navy-950 text-white lg:min-h-[min(92vh,880px)]">
        <img src="{{ asset('images/hero-home.jpg') }}" alt="" class="absolute inset-0 -z-30 size-full object-cover">
        <div class="absolute inset-0 -z-20 bg-gradient-to-b from-navy-950/60 via-navy-950/30 to-navy-950/95"></div>
        <div class="absolute inset-0 -z-10 bg-gradient-to-r from-navy-950/80 via-navy-950/30 to-navy-950/20"></div>

        <div class="mx-auto flex min-h-[760px] w-full max-w-[1320px] flex-col justify-between gap-16 px-7 pb-14 pt-36 lg:min-h-[min(92vh,880px)] lg:px-10 lg:pb-16 lg:pt-40">
            <h1 class="max-w-[17ch] font-display text-[clamp(2.5rem,5.4vw,4.5rem)] font-medium leading-[1.04] tracking-[-0.025em]">
                Segurança que resiste ao escrutínio legal e à operação real.
            </h1>

            <div class="grid gap-12 xl:grid-cols-[1fr_400px] xl:items-end">
                <ul class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ([
                        ...
                        <li class="flex items-center gap-3">
                            <span class="counter grid size-14 shrink-0 place-items-center rounded-full bg-coral-500 font-display text-base font-bold" data-value="{{ $number }}">{{ $number }}</span>
                            <span class="max-w-24 font-mono text-[11px] uppercase leading-4 tracking-[0.05em] text-white/85">{{ $label }}</span>
                        </li>
                    @endforeach
                </ul>

                <div>
                    <p class="max-w-md text-sm leading-6 text-white/75">Engenharia de riscos e conformidade com a legislação angolana — incêndio, segurança eletrónica e SHST para operações que não podem parar.</p>
    ...
    <section class="section-space">
        <div class="container-site">
            <div class="reveal grid gap-8 lg:grid-cols-[1fr_360px]">
                ...
                <p class="text-grey-600">Transformamos segurança de centro de custo em continuidade operacional e vantagem competitiva.</p>
            </div>
            <div class="mt-12 grid gap-px bg-navy-950/10 lg:grid-cols-2">
                @foreach ([
