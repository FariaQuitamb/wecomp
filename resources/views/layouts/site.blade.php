...
<head>
    ...
    <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-E8BPZS43R1"></script>
    <script>
...
<body>
        ...
        <nav class="container-wide flex h-[86px] items-center justify-between transition-all duration-300"
            ...
            <div id="nav-menu"
                ...
                <a href="{{ route('contact') }}"
                    class="button-primary mt-4 lg:hidden">{{ $sitePage->get('header_cta', 'Pedir consultoria') }}</a>
            </div>

            <a href="{{ route('contact') }}"
                class="button-primary hidden lg:inline-flex">{{ $sitePage->get('header_cta', 'Pedir consultoria') }}</a>
        </nav>
    ...
    <section class="relative isolate overflow-hidden bg-navy-950 py-24 text-white">
        ...
        <div class="container-site flex flex-col justify-between gap-8 lg:flex-row lg:items-center">
            <div>
                <h2 class="max-w-[18ch] text-3xl font-semibold md:text-4xl">
                    {{ $sitePage->get('cta_title', 'Vamos avaliar os riscos da sua operação.') }}
                </h2>
                <p class="mt-4 max-w-2xl text-white/70">
                    {{ $sitePage->get('cta_text', 'Consultoria com engenharia HIRA. Primeiro o diagnóstico, depois a proposta.') }}
                </p>
            </div>
            <a href="{{ route('contact') }}"
                class="button-primary shrink-0">{{ $sitePage->get('cta_button', 'Pedir consultoria estratégica') }}</a>
        </div>
    </section>
    ...
    <footer class="bg-navy-950 py-16 text-white/65">
        <div class="container-site">
            <div class="grid gap-10 border-b border-white/15 pb-12 md:grid-cols-2 lg:grid-cols-4">
                <div>
                    ...
                    <p class="mt-4 max-w-xs text-sm leading-6">
                        {{ $sitePage->get('footer_text', 'Engenharia de segurança e conformidade legal para o crescimento do seu negócio.') }}
                    </p>
                </div>
                <div>
                    ...
                    <ul class="mt-4 space-y-2 text-sm">
                        @forelse ($footerSolutions as $solution)
                            <li><a class="hover:text-teal-300"
                                    href="{{ route('solutions.show', $solution) }}">{{ $solution->title }}</a></li>
                        @empty
                ...
                <div>
                    ...
                    <ul class="mt-4 space-y-2 text-sm">
                        @forelse ($footerSectors as $sector)
                            <li><a class="hover:text-teal-300"
                                    href="{{ route('sectors.show', $sector) }}">{{ $sector->title }}</a></li>
                        @empty
            ...
            <div class="flex flex-col justify-between gap-3 pt-7 font-mono text-[11px] md:flex-row">
                <span>© {{ now()->year }}
                    {{ $sitePage->get('copyright', 'Wecomp. Todos os direitos reservados.') }}</span>
